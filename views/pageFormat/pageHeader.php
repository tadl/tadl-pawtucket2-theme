...
<?php
				$vs_home_url = caNavUrl($this->request, '', 'Front', 'Index');
?>
				<a class="navbar-brand tadl-logo-link" href="<?= $vs_home_url; ?>">
					<img src="<?= $this->request->getThemeUrlPath(); ?>/assets/pawtucket/graphics/logo.svg" alt="Traverse Area District Library">
					<span class="tadl-tagline">Local History Collection</span>
				</a>
			</div>
			<div class="tadl-header-search hidden-xs">
				<form class="tadl-search-form" role="search" action="<?= caNavUrl($this->request, '', 'MultiSearch', 'Index'); ?>" aria-label="<?= _t("Search"); ?>">
					...
					<button type="submit" class="tadl-search-button" id="headerSearchButtonDesktop">
						...
						<span class="sr-only"><?= _t("Submit"); ?></span>
					</button>
				</form>
			</div>
			</div>
		</div>
		<div class="tadl-accent-bar"></div>
	</header>
	<nav class="navbar navbar-default yamm tadl-main-nav" role="navigation">
...
?>
	...
	<div class="container tadl-content-shell"><div class="row"><div class="col-xs-12">
